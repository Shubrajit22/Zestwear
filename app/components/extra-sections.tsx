...
export const WhyChooseUs = () => {
  ...
  return (
    <section className="bg-white py-12 sm:py-16 px-4 sm:px-6 md:px-20">
      <div className="max-w-6xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-10 sm:mb-14">
          Why Choose Us
        </h2>
        ...
        <div className="flex justify-between flex-wrap gap-4">
          {features.map((f, idx) => (
            ...
            >
              <f.icon className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-black mb-3 sm:mb-4" />
              <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-800 mb-1 sm:mb-2">{f.title}</h3>
              <p className="text-gray-600 text-xs sm:text-sm md:text-base">{f.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="relative w-full h-[60vh] sm:h-[70vh] md:h-[80vh] lg:h-[90vh]">
        ...
        <div className="absolute inset-0 bg-black/50 flex flex-col justify-center items-center px-6">
          ...
          <motion.p
            ...
            className="text-gray-200 text-center text-base sm:text-lg md:text-xl max-w-3xl"
          >
            From school to events — redefine uniforms with comfort, elegance, and style.
          </motion.p>
          <motion.a
            whileHover={{ scale: 1.05 }}
            className="mt-6 inline-block px-8 py-3 text-lg font-semibold border border-white text-white rounded-md hover:bg-white hover:text-black transition"
            href="#shop"
          >
            Shop Now
          </motion.a>
